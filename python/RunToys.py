...
def getErrorData(N,F):    
    ...
    print "N: ",N, " F: ",F, " L: ", L
    print "Error data output function: ", err_tot_data
    if (err_tot_data==0):
        err_tot_data=.1
    return err_tot_data
...
def getTree(myTree,paramNames,nBins,box):
    ...
    print "entered in function getTree"
    ...
    print "exiting from getTree function"
    return s1
...
def runToys(w,options,cfg,seed):
    ...
    print "passing the obj definition"

    fr.Print("V")
    if options.r>-1:
        extSpBPdf = w.pdf('extSpBPdf')
    
    print "before defining the th1x"
    th1x = w.var("th1x")
    
    print "After defining the th1x"
    ...
    print "We are using the ", unc, " uncertainty"
    print "option.r: ",options.r
    print "seed: ", seed
    ...
    print "creating output drectories (in the file)"
    output.cd()
    print "defining the ttree"
    ...
    iAttempt = -1
    print "Beore looping over toys (iToy)"
    while iToy < options.nToys:
        ...
        if w.var('p0_%s'%options.box)!=None:                
            print "The bad par is: ", "p0_%s"%options.box
            print w.var('p0_%s'%options.box)
        ...
        myTree.Fill()
        iToy+=1        
    print "Ended the loop over iToy"
...
if __name__ == '__main__':
    ...
    if inputFitFile is not None:
        ...
        print "Getting workspace: ", "w"+options.box
        if w.obj("fitresult_extDijetPdf_data_obs") != None:
            fr = w.obj("fitresult_extDijetPdf_data_obs")
        elif w.obj("nll_extDijetPdf_data_obs") != None:
            fr = w.obj("nll_extDijetPdf_data_obs")
