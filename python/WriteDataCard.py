...
def initializeWorkspace(w,cfg,box,scaleFactor=1.,penalty=False,multi=False,x=None,emptyHist1D=None):
    ...
    if w.var('meff_%s'%box).getVal()<0 and w.var('seff_%s'%box).getVal()<0:
        print "value of: meff_%s"%(box)," ", w.var('meff_%s'%box).getVal()
        print "value of: seff_%s"%(box)," ", w.var('seff_%s'%box).getVal()
    ...
    for parameter in parameters:
        ...
        print "This is the function parameter name: ", paramName, " and this is the tot string: ",parameter
        if paramName not in constPars:
            print "Warning, the input list of parameters doesn't match to the expected one!"
    ...
    if emptyHist1D==None:
        print "\nDeclaring empty histo 1D with ",len(x)-1, ", ",x," variable bins"
        emptyHist1D = rt.TH1D("emptyHist1D","emptyHist1D",len(x)-1,x)
        iBinX = -1 # n bin counter? Why don't use again len(x)-1 ?
    ...
    for command in commands:
        lower = command.lower()
        print "Start creating the WorkSpace functions: ", command, " ", lower
        if lower.find('sum::')!=-1 or lower.find('prod::')!=-1 or lower.find('expr::')!=-1 or lower.find('roogaussian::')!=-1 or lower.find('rooefficiency:')!=-1:
            print "Adding function to Workspace via factory"
            w.factory(command)
        else:
            print "Warning! No sum::, prod::, expr::, roogaussian:: or rooefficiency: found in cfg file!"
            ...
            arglist = [name, name]
            print "Function variables list: ", mylist
            for myvar in mylist:
                if w.var(myvar)!=None:
    ...
    w.Print('v')
    if multi:
        print "Defining names for multi pdf scenario\n"
...
def writeDataCard(box,model,txtfileName,bkgs,paramNames,w,penalty,fixed,shapes=[],multi=False):
        ...
        if signals>1:
                ...
                if '2015' in box:
                        ...
                        lumiErrs = [1.062] #using ICHEP one for tests
...
if __name__ == '__main__':
        ...
        if f.lower().endswith('.root'):
            if f.lower().find('resonanceshapes')!=-1:
                ...
                if histoName in names:
                    print "names: ", names
                    print "Getting histogram ", histoName, " from file ", rootFile
                    rootFile_tmp = rootFile
    ...
    print myTH1
    #myTH1 = rootFile_tmp.Get(histoName_tmp)
    print myTH1
    ...
    signalHistosRebin = []
    signalHistos = []
    print "Opening ", rootFile_tmp," for resonance shapes" 
    signalFile = rootFile_tmp#rt.TFile.Open(rootFile)#signalFileName)
    names = [k.GetName() for k in signalFile.GetListOfKeys()]
    print "Names for signalFile ", names
    for name in names:
        d = signalFile.Get(name)
        print "Examining object: ", name, " ", d
        print "Looking for: signalHisto_%i"%(massPoint) 
        if isinstance(d, rt.TH1):
            #d.SetDirectory(rt.gROOT)
            if name=='signalHisto_%i'%(massPoint):#'h_%s_%i'%(model,massPoint):
                print "Found: signalHisto_%i"%(massPoint) 
    ...
    outFile = 'dijet_combine_%s_%i_lumi-%s_%s.root'%(model,massPoint,luminosity,box)#lumi/1000.,box)
