...
def getFileName(hybridLimit, massPoint, box, model, lumi,  directory, method, t):
    ...
    if hybridLimit == "higgsCombineToys" or t>0:
        fileName = "%s/%s%s_%s_lumi-%s_%s.%s.mH120.%s.root"%(directory,hybridLimit,model,massPoint,luminosity,box,method,t)
    else:
        fileName = "%s/%s%s_%s_lumi-%s_%s.%s.mH120.root"%(directory,hybridLimit,model,massPoint,luminosity,box,method)
    return fileName
...
if __name__ == '__main__':
    ...
    print "===>>>",options.mass
    print "len(options.mass.split(','))", len(options.mass.split('_'))
    ...
    for massPoint in massPoints:#massIterable:
